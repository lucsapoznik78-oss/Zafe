  ...
  if (!competition) {
    return (
      <div className="py-20 text-center text-muted-foreground">
        <Trophy size={40} className="mx-auto mb-3 text-green-400/40" />
  ...
  return (
    <div className="py-6 space-y-5">
      ...
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          <h1 className="text-xl font-bold text-white flex items-center gap-2">
            <Trophy size={20} className="text-green-400" /> {competition.name}
          </h1>
        ...
        {participant ? (
          <div className="text-right shrink-0">
            <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-green-400/20 border border-green-400/30 text-[10px] font-bold text-green-400">
              <CheckCircle2 size={11} /> Inscrito
            </span>
            <p className="text-2xl font-bold text-green-400 mt-1">{myRow?.points ?? 0} pts</p>
            <p className="text-[11px] text-muted-foreground">
              {myRow ? `${myRow.posicao}º lugar` : "Sem pontuação ainda"} ·{" "}
              <Link href="/copa/ranking" className="text-green-400 hover:underline">
                ver ranking
              </Link>
            </p>
          </div>
      ...
      <div className="flex items-center gap-3 flex-wrap">
        ...
        <Link
          href="/copa/ranking"
          className="px-3 py-1.5 rounded-md text-sm text-muted-foreground hover:text-white border border-border hover:border-green-400/40 transition-all flex items-center gap-1.5"
        >
