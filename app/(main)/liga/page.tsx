  ...
  if (error || !topics || topics.length === 0) {
    return (
      <div className="text-center py-20 text-muted-foreground">
        ...
        <p className="text-sm">
          {isEncerrados ? "Eventos resolvidos aparecerão aqui" : "Tente outros filtros"}
        </p>
      </div>
    );
  }
...
export default async function LigaPage({ searchParams }: PageProps) {
  ...
  const buildHref = (key: string) =>
    `/liga?tab=${key}${sort !== "popular" ? `&sort=${sort}` : ""}${category ? `&category=${category}` : ""}${search ? `&search=${search}` : ""}`;
  ...
  return (
    <div className="py-6 space-y-5">
      ...
      >
        <LigaList sort={sort} category={category} search={search} tab={tab} />
      </Suspense>
