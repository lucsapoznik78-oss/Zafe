...
export default async function AdminPage() {
  ...
  const totalCommission = statsData?.reduce((sum: number, w: { balance: number }) => sum + w.balance, 0) ?? 0;
  ...
  return (
    <div className="py-6 space-y-6 max-w-4xl mx-auto">
      ...
      <AdminStats totalCommission={totalCommission} pendingCount={pending?.length ?? 0} toResolveCount={toResolve?.length ?? 0} />
