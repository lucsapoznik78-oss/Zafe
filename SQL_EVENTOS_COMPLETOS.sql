-- ============================================================
-- AUMENTAR EVENTOS: LIGA (20) + CONCURSO (15) - SEM "TESTE"
-- RODAR NO DASHBOARD: https://supabase.com/dashboard/project/mhckuhqyyfoapzgrqeco/sql/new
-- ============================================================
...
BEGIN
  ...
  -- ============================================================
  -- LIGA: 20 EVENTOS (sem concurso_id)
  -- ============================================================
  ...
  VALUES
    ...
    (admin_id, 'A Petrobras vai anunciar novo aumento de combustíveis?', 'Pressão do mercado internacional e câmbio forçam a estatal a revisar preços.', 'economia', 'active', 1, '2026-05-14 12:00:00+00', false),
    ...
    (admin_id, 'O impeachment de Trump avança no Congresso americano?', 'Processo político ganha tração com novas evidências. Câmara aprova prosseguimento?', 'politica', 'active', 1, '2026-05-14 18:00:00+00', false),
  ...
  RAISE NOTICE 'Eventos da Liga inseridos (sem "Teste")!';
  
  -- ============================================================
  -- CONCURSO: 15 EVENTOS (com concurso_id, min_bet=20)
  -- ============================================================
  IF concurso_ativo_id IS NOT NULL THEN
    ...
    RAISE NOTICE 'Eventos do Concurso inseridos (15 eventos, sem "Teste")!';
  ELSE
    RAISE NOTICE 'Nenhum concurso ativo encontrado. Eventos do Concurso NÃO foram inseridos.';
  END IF;
  
  -- ============================================================
  -- LIMPAR EVENTOS ANTIGOS COM "TESTE" NO NOME (se existirem)
  -- ============================================================
...
-- ============================================================
SELECT 
  CASE WHEN concurso_id IS NULL THEN 'LIGA' ELSE 'CONCURSO' END AS tipo,
  COUNT(*) AS total_eventos,
  COUNT(*) FILTER (WHERE status = 'active') AS ativos,
  MIN(closes_at) AS primeiro_fecha,
  MAX(closes_at) AS ultimo_fecha
FROM topics
WHERE created_at >= NOW() - INTERVAL '1 hour'
GROUP BY (CASE WHEN concurso_id IS NULL THEN 'LIGA' ELSE 'CONCURSO' END);
...
SELECT 
  ...
  status,
  closes_at,
  CASE WHEN concurso_id IS NOT NULL THEN 'SIM' ELSE 'NÃO' END AS is_concurso
FROM topics
WHERE status = 'active'
ORDER BY closes_at
LIMIT 50;
