        ...
        return (
          <div
            ...
            className={`grid grid-cols-[2.5rem_1fr_3.5rem_3.5rem_3.5rem] gap-2 px-4 py-2.5 items-center border-b border-border last:border-b-0 text-sm ${
              isMe ? "bg-green-400/10" : ""
            }`}
          >
            <span className="font-mono text-muted-foreground flex items-center gap-1">
              {r.posicao}
              {r.posicao === 1 && <Trophy size={11} className="text-green-400" />}
            </span>
            <span className={`truncate ${isMe ? "text-green-400 font-semibold" : "text-white"}`}>
              {r.username}
              {isMe && <span className="text-[10px] ml-1.5 text-green-400/70">(você)</span>}
            </span>
