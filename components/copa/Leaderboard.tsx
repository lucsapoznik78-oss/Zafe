        ...
        return (
          ...
          >
            <span className="font-mono text-muted-foreground flex items-center gap-1">
              {r.posicao}
              {r.posicao === 1 && <Trophy size={11} className="text-yellow-400" />}
            </span>
            <span className={`truncate ${isMe ? "text-yellow-400 font-semibold" : "text-white"}`}>
              {r.username}
              {isMe && <span className="text-[10px] ml-1.5 text-yellow-400/70">(você)</span>}
            </span>
