...
### New this round
- **H1 `[NEW]` — Annual-limit bypasses (CMN 5.298).** (a) Allies never limit-checked (`criar/route.ts:46-51` only pairs creator↔adversary); (b) `apostas-privadas/[id]/convidar/route.ts:52-56` adds participants with no limit check; (c) TOCTOU between `verificarLimiteAnual` and debit lets two concurrent bets both pass. Cap is evadable + not concurrency-safe. (privadas.)
- **H2 `[NEW]` — `replicar-topics` clones full topic row incl. resolution/oracle fields.** `route.ts:52-59` spreads `...topic`, only nulls `id`; a resolved/oracle-stateful source corrupts the contest clone. Whitelist columns. (concurso.)
...
- **H6 `[NEW]` — PostgREST `.or()` filter injection.** `app/api/ligas/buscar-usuarios/route.ts:20` interpolates user `q` raw into an `.or()` string on the **admin (RLS-bypassed) client**. Commas/parens/`.eq` break out of the `ilike`. Also `amigos/bloquear/route.ts:17-19` interpolates ids into `.or()`. (security, social.)
...
- **H8 `[NEW]` — Unvalidated bet input.** `comunidade/[id]/palpitar/route.ts:10-12` passes `side`/`amount` to the wallet path with no validation; `apostar/route.ts:15` & `concurso/palpitar/route.ts:12` check `<=0` but not `Number.isFinite` → `"abc"`/`Infinity` reach odds math + CAS. (api.)
- **H9 `[NEW]` — Quoted payout overstates real parimutuel return.** `components/topicos/BetForm.tsx:29-35` omits the user's own stake from the pool denominator → e.g. pools 100/100, bet 100 shows Z$200 but real payout is Z$150 (33% inflated). (odds.)
- **H10 `[NEW]` — `aprovar` seed bets break Z$ conservation.** `admin/aprovar/route.ts:32-54` inserts Z$1/side seed bets on the **admin's own account** with **no wallet debit** → Z$ minted into the pool; admin also collects payout (original #48 confirmed open + worse). (admin.)
- **H11 `[NEW]` — Admin DAU metric reads a non-existent column.** `app/admin/page.tsx:54` filters `profiles.updated_at` which doesn't exist → "Ativos (30 dias)" tile is meaningless; no activity/retention instrumentation exists at all. (analytics.)
...
- **H13 `[NEW]` — `resolver-direto` verdict mapping is index-positional.** `route.ts:124,164` keys verdicts by 1-based prompt position; an omitted/renumbered item from Claude misroutes a verdict to the wrong market or silently → INCERTO. Mixed `bets`+`concurso_bets` topics also leave one set unpaid. (resolver.)
...
7. **H1 / 57** — Close annual-limit bypasses (allies, convidar, TOCTOU) and switch to rolling 365-day window. *(next — first open HIGH)*
