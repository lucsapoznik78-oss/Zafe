...
## 🚨 INCIDENTE 2026-06-15 — Comunidade não fecha / criador não resolve (cron morto)

**Sintoma (reportado pelo dono):** eventos da Comunidade não estavam fechando e o criador não conseguia decidir o resultado.

**Causa raiz — TODOS os crons estão 401 (`CRON_SECRET` ausente).** `lib/cron-auth.ts:8` faz `if (!secret) return false`; não há `CRON_SECRET` em nenhum `.env` do repo nem (presumivelmente) na Vercel. Logo o cron `comunidade-fechar` (`vercel.json:14`, `0 3 * * *`) responde 401 toda vez e **nunca move os eventos `active` expirados → `awaiting_resolution`**. Como o botão "Resolva seu evento" (`ResolveForm`) só aparece em `awaiting_resolution`, o criador nunca recebe o prompt. (É o `H14`/`#30` se manifestando; afeta os 15 crons: fechar/resolver mercados, order-matching, bônus semanal, ciclo do concurso, Copa, `comunidade-abandonados/-contestacoes/-snapshots`.)

**Confirmado na produção (DB `mhckuhqyyfoapzgrqeco`, 2026-06-15):** 4 eventos expirados (fecharam 13–14/jun) presos em `active`, com `resolution_deadline = closes_at+72h` gravado na *criação* (prova de que o cron nunca os tocou); 1 evento em `awaiting_resolution` movido quando o cron ainda rodava (~13/jun). **Schema saudável** — `notifications.title/body/data` existem, enums têm `market_resolved`/`commission`/`bet_won`, `v_community_event_stats` e `wallets.version` presentes → não é problema de banco.

**✅ Mitigação aplicada 2026-06-15 (LIVE DB):** os 4 eventos expirados foram destravados manualmente espelhando o cron — `status='awaiting_resolution'`, `resolution_deadline=now()+72h`, + 1 notificação `market_resolved` por criador (4 eventos, 4 notificações). Estado pós-fix: 0 expirados travados, 5 em `awaiting_resolution`. Os criadores já podem resolver.

**⛔ Ação pendente do dono (única coisa que destrava de vez):** definir `CRON_SECRET` nas env vars da Vercel (a Vercel injeta `Authorization: Bearer <CRON_SECRET>` nas chamadas de cron automaticamente quando a var existe). Sem isso, TODOS os crons seguem mortos e o problema reaparece a cada novo evento. Recomendado também: deployar o fallback `isExpiredActive` (já na working tree — `app/(main)/comunidade/[id]/page.tsx:64`, `app/api/comunidade/[id]/resolver/route.ts:29`) para o criador resolver imediatamente no fechamento, sem depender do cron diário.

**Hardening sugerido:** `verifyCronAuth` falhar fechado em silêncio (só um 401 no log) faz um deploy mal configurado desligar toda a automação sem alarme — adicionar um healthcheck/alerta quando um cron não roda há > 24h.

---

## 🆕 RE-AUDIT 2026-06-15 — core + money sweep (NEW findings `G1–G22`)

> Re-run requested "run all agents, order by importance." **Completed: the 7 highest-importance agents** — `qa`, `security`, `db`, `compliance`, `wallet`, `resolver`, `odds` (the latter three finished via direct code analysis after the subagent quota was hit mid-run). Each cross-checked against this report (last fix pass 2026-06-11); only NEW items or regressions are listed. Focus landed on the **new code since 2026-06-11**: the Copa module (`migrations 027/028/037`, `lib/copa/*`), the Premium module (`migration 039`, `lib/premium/*`, insights route), and the KYC/admin routes.
>
> ⚠️ **Deferred (platform usage limit, resets 04:20 America/Sao_Paulo):** the lower-priority module/infra/utility/event-integrity agents — `liga, economico, privadas, concurso, auth, admin, api, deploy, perf, frontend, social, analytics, docs, migration, tests, content, crosscheck, validaty, dates`. Most of their domains are already covered by the findings below (concurso→G2, economico/odds→G3/G4/G10/G11/G15/G16, api/auth/admin→G1/G6/G7/G12). Re-run them after the limit resets for the full 26-agent sweep.
>
> ✅ **Confirmed holding (no regression):** wallet CAS helpers; Copa money SQL (`copa_buy_in`/`copa_payout`/`copa_refund_all` — atomic SECURITY DEFINER, row-locked, service-role-only); `N1–N12`, `C1–C8`, `H3`, `H4`, `H9`, 18+ gate, `admin/usuarios/[id]/ajustar` CAS; `npx tsc --noEmit` clean. **Compliance: no NEW violations** — the "depósito/cassino" matches are all compliant negations ("Sem depósito", "Não é cassino"); `N21` (layout metadata "Fantasy Game") remains an owner decision.

### 🔴 CRITICAL
- **G1 — `profiles` RLS allows privilege escalation (admin takeover / paywall / KYC bypass).** `supabase/migrations/001_initial.sql:219` — `profiles_own_update` is `FOR UPDATE USING (auth.uid() = id)` with **no `WITH CHECK` and no column-level grant**, so any authenticated user can write arbitrary columns on their own row straight from the browser Supabase client: `is_admin=true` (full admin: approve/resolve markets, adjust any wallet, finalize Copa payout), `is_premium=true` (G5), `cpf`/`kyc_verified` (G7). **Fix:** `REVOKE UPDATE` on `profiles` from `authenticated` and `GRANT UPDATE(username, full_name, avatar_url, birth_date, …)` only (exclude `is_admin`, `is_premium`, `premium_until`, `kyc_verified`, `cpf`, `banned`), plus a `WITH CHECK (auth.uid() = id)`; privileged columns become service-role-only. (security, db, auth, admin, wallet.)
- **G2 — Concurso payout double-pay (no atomic claim).** `lib/concurso-payout.ts:54-98,119-147` — `pagarConcursoBets`/`...Multi` are not guarded by `claimTopicForResolution` (C7 only claims the `bets` table). For a concurso topic the oracle calls `pagarVencedores` first, which returns early `skipped_concurso_topic` **without claiming the topic**, so it is never atomically locked before concurso payout. Two resolvers racing the same `status='resolving'` topic (cron `resolver-oracle` + admin `resolver-direto`, or two cron fires) both read `concurso_bets WHERE status='matched'` then both `creditConcursoBalance` the winners → **ZC$ double-pay, breaks Z$ conservation (rule 4)**. The `status='matched'` filter is read-then-write, not an atomic claim. **Fix:** gate concurso payout behind a row-level atomic claim (claim the topic, or guard each bet flip with `.eq('status','matched').select()` row-count), mirroring C7. (resolver, concurso, wallet.)
- **G3 — Order-book refund mints Z$.** `lib/payout.ts:94-101` (`refundBets`←`reembolsarTodos`) — an order-book BUY position is stored in `bets` with `amount = quantity` (full face, e.g. 100 Z$) but the buyer paid only `price*quantity` (e.g. 30 Z$, which the seller already pocketed at trade time via `execute_trade`). On a market refund the buyer is credited the full `bet.amount` (100 Z$) → **mints ~70 Z$ from nothing, breaks rule 4**. **Fix:** store a `cost_basis = price*quantity` on traded positions and refund the cost basis, not face value. (wallet, odds, resolver.)
- **G4 — Order-book positions weighted at face value in the parimutuel pool.** `lib/payout.ts:198-206,282-305` + `migration 033:109-112` — pools sum `bet.amount` (face). A buyer who paid 30 Z$ enters the win pool weighted at 100 Z$, so the pool can pay out more Z$ than was ever debited (their real 30 Z$ went to the seller, not the pool). **Fix:** weight traded positions by cost basis, or settle secondary-market positions against the original primary stake. (odds, wallet.)

- **G23 — [OWNER DECISION 2026-06-16] Premium pay-to-win em contexto de prêmio real (Concurso).** `app/(main)/concurso/[id]/page.tsx` agora renderiza `<TopicInsights>` com o **mesmo gate Premium da Liga** (a pedido explícito do dono, opção "B" após aviso de compliance). Efeito: o assinante **pago** Premium recebe pesquisa/insight de IA exclusivo nos eventos do **Concurso Mensal**, que paga **prêmio real em R$ via PIX**. Isso cria vantagem **pay-to-win numa competição de prêmio real** — exatamente o que o agent `zafe-prize-integrity` existe para impedir ("uma falha aqui pode descaracterizar o concurso gratuito e encostar em loteria", Lei 14.790/2023, Art. 49 — fantasy sport exige habilidade/footing igual). **Risco:** pode descaracterizar o enquadramento "competição de habilidade com footing igual" (modelo Cartola FC) e aproximar de loteria/jogo de azar. **Mitigação compliant disponível e recomendada (opção "A", recusada):** liberar o insight para TODOS os participantes do concurso (sem cadeado) → footing igual, sem vantagem paga. **Ação recomendada:** validar com o jurídico antes de manter em produção; se mantido gated, documentar a justificativa de enquadramento. (compliance, concurso, prize-integrity.)

### ⛔🟠 HIGH
- **G5 — Premium self-grant.** `migration 039:9` + `001_initial.sql:219` — same unrestricted `profiles` UPDATE lets any user set `is_premium=true` client-side, bypassing the paywall and unlocking the premium AI-insights perk for free. **Fix:** exclude `is_premium`/`premium_until` from the authenticated grant (subset of G1). (security, db.)
- **G6 — Premium insights = cost-amplification DoS.** `app/api/topicos/[id]/insights/route.ts:50` — `getOrGenerateInsights` runs two paid Claude + web-search calls on any cache miss **before** the premium/auth gate (and `user` may be `null`), so anyone can iterate `topic` ids to force unbounded platform-funded AI generations. **Fix:** only generate for authenticated premium callers (or a server cron); free/anon strictly read existing cache; add rate-limiting. (security, api, qa.)
- **G7 — KYC self-asserted + client-writable + racy.** `app/api/kyc/route.ts:31` self-sets `kyc_verified=true` for any CPF that passes the check-digit algorithm (no real identity check); worse, the unrestricted `profiles` UPDATE lets a user write `cpf`/`kyc_verified` directly from the client, bypassing the route entirely; and uniqueness is a check-then-update with **no UNIQUE constraint** (two concurrent requests register the same CPF → Sybil bypass for the R$ contest). **Fix:** move `cpf`/`kyc_verified` writes to service-role-only, add a partial `UNIQUE` index on `profiles(cpf)`, integrate a real CPF/identity provider. (security, api, concurso, compliance.)
- **G8 — Copa `applyMatchResult` non-atomic → silent under-scoring.** `lib/copa/resolve.ts:64-105` sets `copa_matches.status='finished'` **before** `copa_rescore_match`; if the rescore RPC fails the match is left `finished` with zero score events and `resolveDueMatches` only re-picks `scheduled`/`postponed`, so it is never retried. **Fix:** flip to `finished` only after `copa_rescore_match` succeeds (or wrap both in one RPC). (resolver.)
- **G9 — AI oracle resolves on a single source.** `lib/oracles/ai-triple-check.ts:181-183` returns `confianca:"alta"` and pays out on **check1 alone** — check2/check3 are skipped when check1 is decisive, and the parsed confidence is never compared to any threshold. CLAUDE.md / Layer-2 spec require **two independent agreeing searches, each ≥0.85**. **Fix:** require check1 ∧ check2 agreement and enforce the ≥85% gate before resolving. (resolver, compliance.)
- **G10 — Order-book fill bookkeeping race.** `lib/order-matching.ts:106-127` updates `filled_qty`/`status` with raw `.update().eq("id")`, no CAS / status precondition; the immediate `tryMatchOrders(newOrderId)` and the book-sweep pass (`ordem/route.ts:173-183`) can write stale fills for the same counter order. **Fix:** update fills inside the `execute_trade` RPC, or CAS on `filled_qty`. (odds, economico.)
- **G11 — Cancel over-refunds escrow on stale `filled_qty`.** `app/api/topicos/[id]/ordem/[orderId]/route.ts:19-55` computes the escrow refund from the `filled_qty` read **before** the guarded cancel; a concurrent partial fill in between → over-refund of the filled portion's escrow. **Fix:** compute the refund from the cancelled row's returned current `filled_qty`. (odds, wallet.)

### ⛔🟡 MEDIUM
- **G12 — Copa admin `fill_slot` has no kickoff guard.** `app/api/admin/copa/partidas/[id]/route.ts:59-68` lets `home_team`/`away_team` be changed when status is `under_review`/`postponed` or already kicked off (only `finished` is blocked) → teams can change after predictions locked, altering how stored picks score. **Fix:** reject `fill_slot` once `kickoff_at <= now()` or status ≠ `scheduled`. (admin.)
- **G13 — Copa group-pick scoring not idempotent.** `lib/copa/group-picks.ts:150-156` does a plain `.insert(events)` after a delete (unlike `copa_rescore_match`'s `ON CONFLICT DO NOTHING`), so two concurrent resolver runs on the group anchor match hit the `UNIQUE(match_id,participant_id,reason)` constraint and abort scoring. **Fix:** upsert / `ON CONFLICT DO NOTHING`. (resolver.)
- **G14 — Layer-4 concurso refund non-idempotent.** `lib/oracles/index.ts:84-89` — after `MAX_RETRIES`, `reembolsarTodos` claims `bets` but the following `pagarConcursoBets(...,"cancelled")` is unclaimed; concurrent cron fires both refund `concurso_bets WHERE status='matched'` → double ZC$ refund (same root cause as G2 on the refund branch). **Fix:** claim/guard the per-bet status flip before refunding. (resolver, concurso.)
- **G15 — Probability snapshots never written on trades/bets.** `topic_snapshots` is inserted only by the close cron (`cron/fechar-mercados/route.ts:114-120`), never after an order-book trade or a new bet, so the chart is flat until close and stale probabilities feed `BetForm` payout quotes. **Fix:** insert a snapshot inside the trade/bet path (or `execute_trade`). (odds.)
- **G16 — `v_topic_stats` counts non-live bets.** `004_fix_stats_view.sql:26` sums bets with `status NOT IN ('refunded')`, so `exited`/`won`/`lost` still inflate `volume_*`/`prob_*`; the order-book buyer-row + seller `exited`-row interaction transiently double-counts the same face value. **Fix:** restrict the view to live statuses (`pending`,`matched`,`partial`). (odds, db; extends #45.)
- **G17 — Inconsistent "live bet" status sets.** `lib/apostar.ts:62` (`NOT IN ('refunded','lost')`) vs `:129` (`'refunded','exited','lost','won'`) vs `ordem/route.ts:68` (`'refunded','lost','exited'`) — three different filters; e.g. an `exited` opposite-side bet still wrongly blocks a new bet. **Fix:** one canonical live-status set reused everywhere. (qa, economico.)
- **(still open) N17 — economia oracle pt-BR number parsing.** `lib/oracles/economia.ts:21,152,227` `.replace(".","").replace(",",".")` mangles English-format thresholds ("1.5"→15) → wrong auto-resolution/payout side. Confirmed unfixed. (resolver, economico.)
- **(still open) `validacaoAI` fail-open.** `lib/oracles/ai-triple-check.ts:219` returns `true` on exception, gating the API-fixa cross-check open. (resolver.)

### ⛔🔵 LOW
- **G18 — `COPA_BUY_IN` constant wrong.** `lib/copa/types.ts:7` `COPA_BUY_IN = 650` contradicts the real Z$ 400 buy-in (migration `027` default/seed and the value actually debited); currently unused, latent footgun if a UI imports it. **Fix:** set to 400 or derive from `competition.buy_in`. (qa.)
- **G19 — `calcOdds` 999x cap leaks to users.** `lib/odds.ts:11` returns `999.0` for the empty side; `executePalpitar` stores `locked_odds=999`/`potential_payout=amount*999` and surfaces "probabilidade estimada 0%" in the transaction text (`apostar.ts:118`) on a market's first bet. **Fix:** cap implied odds at a saner bound (e.g. 100x) and special-case first-bet copy. (odds.)
- **G20 — Traded-position `locked_odds`/`potential_payout` misrepresent return.** `migration 033 execute_trade:43,111-112` writes these but parimutuel payout ignores them, yet they're shown in portfolio. **Fix:** stop displaying `potential_payout` for traded positions or compute it from the live pool. (odds.)
- **G21 — `cpf`/`kyc_verified` columns only in `RODAR_NO_SUPABASE.sql`, not a numbered migration** → a fresh numbered-migration replay yields a schema where the KYC route writes non-existent columns. **Fix:** add `ADD COLUMN IF NOT EXISTS` for both to a numbered migration. (db.)
- **(still open) `salvarResolucao` swallows insert failures.** `lib/oracles/index.ts:52-54` logs only → the `resolucoes` audit trail can silently lose rows while payouts proceed. (resolver.)

### Recommended order (this sweep)
1. **G1** — lock down the `profiles` UPDATE policy (one migration); it is the root of G5/G7 and an admin-takeover vector.
2. **G2 + G14** — atomic claim/guard on concurso payout & refund (active ZC$ double-pay).
3. **G3 + G4** — order-book cost-basis (active Z$ minting / pool imbalance); also fixes G15/G16 displayed-prob drift.
4. **G6** — gate the insights generator behind premium+auth (live cost-amplification).
5. **G7** — service-role-only KYC writes + UNIQUE(cpf) (contest integrity / R$ exposure).
6. **G8/G9/G10/G11** — Copa scoring retry, two-source AI gate, fill-race, cancel over-refund.
7. Rest as convenient. **Then re-run the deferred 19 agents after the usage limit resets.**
...
- **🟡 7.** Service role key. ✅ **Parte do hardcode RESOLVIDA (verificado 2026-06-15):** `scripts/seed-eventos.mjs:3-4` agora lê `SUPABASE_SERVICE_ROLE_KEY` de env var (não há mais chave hardcoded em código; `repomix-output.txt` só contém a anon key pública). ⛔ **RESIDUAL aberto:** `.env.local` (gitignored) ainda guarda em texto puro o **`service_role` JWT (exp 2090), `VAPID_PRIVATE_KEY`, `FIRECRAWL_API_KEY`** — e o `CRON_SECRET` foi compartilhado em chat nesta sessão. **Rotação ainda pendente** (ação no dashboard Supabase/Vercel — fora do alcance do agente). (deploy, security.)
